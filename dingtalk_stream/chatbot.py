...
def reply_specified_single_chat(
    user_id: str, user_nickname: str = ""
) -> ChatbotMessage:
    d = {
        "senderId": user_id,
...
class ChatbotHandler(CallbackHandler):
    ...
    def reply_markdown_card(
        ...
        :return:
        """
        markdown_card_instance = MarkdownCardInstance(
    ...
    def reply_rpa_plugin_card(
        ...
        rpa_plugin_card_instance.reply(
            plugin_id,
            plugin_version,
            plugin_name,
            ability_name,
    ...
    def reply_markdown_button(
        ...
        :return:
        """
        markdown_button_instance = MarkdownButtonCardInstance(
            self.dingtalk_client, incoming_message
        )
    ...
    def reply_ai_markdown_button(
        ...
        :return:
        """
        markdown_button_instance = AIMarkdownCardInstance(
            self.dingtalk_client, incoming_message
        )
        markdown_button_instance.set_title_and_logo(title, logo)

        markdown_button_instance.ai_start(
            recipients=recipients, support_forward=support_forward
        )
        markdown_button_instance.ai_streaming(markdown=markdown, append=True)
        markdown_button_instance.ai_finish(
            markdown=markdown, button_list=button_list, tips=tips
        )

        return markdown_button_instance

    def reply_carousel_card(
        self,
        incoming_message: ChatbotMessage,
        markdown: str,
        image_slider,
        ...
        :return:
        """
        carousel_card_instance = CarouselCardInstance(
            self.dingtalk_client, incoming_message
        )
    ...
    def ai_markdown_card_start(
        ...
        :return:
        """
        ai_markdown_card_instance = AIMarkdownCardInstance(
            self.dingtalk_client, incoming_message
        )
    ...
    def get_image_download_url(self, download_code: str) -> str:
        ...
        try:
            ...
            self.logger.error(
                f"get_image_download_url, error={e}, response.text={response_text}"
            )
            return ""
    ...
    def set_off_duty_prompt(self, text: str, title: str = "", logo: str = ""):
        ...
        prompt_card_data = generate_multi_text_line_card_data(
            title=title, logo=logo, texts=[text]
        )
        ...
        try:
            ...
            self.logger.error(
                f"set_off_duty_prompt, error={e}, response.text={response_text}"
            )
    ...
    def reply_text(self, text: str, incoming_message: ChatbotMessage):
        ...
        try:
            ...
            self.logger.error(
                f"reply text failed, error={e}, response.text={response_text}"
            )
            return None
        return response.json()
    ...
    def reply_card(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.error(
                f"reply card failed, error={e}, response.text={response_text}"
            )
            return ""
    ...
    def update_card(self, card_biz_id: str, card_data: dict):
        ...
        try:
            ...
            self.logger.error(
                f"update card failed, error={e}, response.text={response_text}"
            )
            return response.status_code
        return response.json()
